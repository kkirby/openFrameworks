...
void ofApp::setup(){
	...
	show.append(
		L"The openFrameworks API is constantly evolving: any corrections, additions or comments are very welcome!");
...
void ofApp::draw(){
	...
	ofSetColor(0, 0, 0, 255);
	faceNavi.drawString(
		"Rendering mode [Space]: " + ofToString(bitmapRendering ? "Bitmap" : "Shapes"),
		50,
		ofGetHeight() - 80);
	faceNavi.drawString(
		"Wrap-mode [key w]: " + ofToString(
			face.getWordWrap() ? "Word-wrap" : "Character-wrap"),
		50,
		ofGetHeight() - 50);
	faceNavi.drawString(
		"Alignment-mode [key e]: " +
		ofToString(face.getAlignByPixel() ? "Alignment by Pixel" : "Optimized"),
		50,
		ofGetHeight() - 20);
