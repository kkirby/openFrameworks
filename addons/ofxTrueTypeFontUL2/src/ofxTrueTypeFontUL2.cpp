...
#define NOT_START_OF_LINE_CHARS	\
	L"%}¢°‰′″℃゛゜ゝゞヽヾ！％），．：；？］｡｣､･ｧｨｩｪｫｬｭｮｯｰﾞﾟ￠,)]｝、〕〉》」』】〙〗〟’”｠»ーァィゥェォッャュョヮヵヶぁぃぅぇぉっゃゅょゎゕゖㇰㇱㇲㇳㇴㇵㇶㇸㇹㇷㇷ゚ㇺㇻㇼㇽㇾㇿ々〻‐゠–〜～　 ?!‼⁇⁈⁉・:;/。.\n"
...
namespace ul2_ttf_utils {
	...
	template <class T> wstring convToUCS4(basic_string<T> src){
		wstring dst = L"";
...
static bool loadFontFace(string fontname, int _fontSize, FT_Face& face, string& filename){
	...
	if(err){
		...
		ofLogError("ofTrueTypeFont") << "loadFontFace(): couldn't create new face for \"" << fontname <<
			"\": FT_Error " << err << " " << errorString;
...
class ofxTrueTypeFontUL2::Impl {
	public:
		...
		bool implLoadFont(string filename, float fontsize, bool _bAntiAliased, bool makeContours, float _simplifyAmt,
						  int dpi, bool useTexture, string scriptTagName);
		bool implLoadSubFont(string filename, float sizeRate, float baseLineRate, int unicodeRangeStart,
							 int unicodeRangeEnd, string scriptTagName);
		bool implLoadSubFontVal(string filename, float fontsize, float baseLine, int unicodeRangeStart,
								int unicodeRangeEnd, string scriptTagName);
		...
		template <class T> void commonLayouts(wstring src, float x, float y, float width, float height, int textAlign,
											  ul2_rendering_types type, T& result);
		template <class T> void commonLayouts2(wstring src, float x, float y, float width, float height,
											   ul2_text_align textAlign, vector<ofRectangle>* lineWidthList,
											   ul2_rendering_types type, T& result);

		void addOTFeature(const char* feature_tag, unsigned int value, unsigned int start = 0,
						  unsigned int end = static_cast<unsigned>(-1));
...
bool ofxTrueTypeFontUL2::Impl::implLoadFont(string filename, float fontsize, bool bAntiAliased, bool makeContours,
											float simplifyAmt, int dpi, bool useTexture, string scriptTagName){
	...
	if(lineHeight_ == 0){
		lineHeight_ = baseFontSize_ * 2.2f;
	}
...
bool ofxTrueTypeFontUL2::Impl::implLoadSubFontVal(string filename, float fontsize, float baseLine,
												  int unicodeRangeStart, int unicodeRangeEnd, string scriptTagName){
	ul2_face_info set;
	...
	set.hasUnicodeRange = set.unicodeRangeStart <= set.unicodeRangeEnd &&
						  (set.unicodeRangeStart != 0 || set.unicodeRangeEnd != 0);
	...
	float fontUnitScale = ((float)set.fontsize * dpi_) / (72 * set.face->units_per_EM);
	set.ascenderHeight = set.face->ascender * fontUnitScale;
	set.descenderHeight = set.face->descender * fontUnitScale;
...
bool ofxTrueTypeFontUL2::Impl::implLoadSubFont(string filename, float sizeRate, float baseLineRate,
											   int unicodeRangeStart, int unicodeRangeEnd, string scriptTagName){
	return implLoadSubFontVal(
		filename,
		baseFontSize_ * sizeRate,
		baseFontSize_ * baseLineRate,
		unicodeRangeStart,
		unicodeRangeEnd,
		scriptTagName);
}

void ofxTrueTypeFontUL2::Impl::addOTFeature(const char* feature_tag, unsigned int value, unsigned int start,
											unsigned int end){
	hb_tag_t add = hb_tag_from_string(feature_tag, -1);
	for(int i = 0; i < m_featuresnum; i++){
		if((m_features + i)->tag == add){
			return;
		}
	}
...
vector<ul2_string_layouts_info> ofxTrueTypeFontUL2::Impl::getHbPosition(wstring wsrc){
	...
	for(i = 0; i < len; ){
			...
			for(k = facenum - 1; k > 0; k--){
				if(fBuffs[k].info->codepoint != 0){
					if(faces[k].hasUnicodeRange == 1){
						if(faces[k].unicodeRangeStart <= src[i] && src[i] <= faces[k].unicodeRangeEnd){
							break;
						}
					}
					else {
				...
				for(t = facenum - 1; t > 0; t--){
					if(FT_Get_Char_Index(faces[t].face, src[i + 1]) > 0){
						if(faces[t].hasUnicodeRange == 1){
							if(faces[t].unicodeRangeStart <= src[i + 1] && src[i + 1] <= faces[t].unicodeRangeEnd){
								break;
							}
						}
						else {
		...
		v.breakable = (ul2_ttf_utils::SetNotEOL.find(src[i]) == string::npos &&
					   (ul2_ttf_utils::isCJK(src[i]) || (i < len - 1 ? ul2_ttf_utils::isCJK(src[i + 1]) : true) ||
						ul2_ttf_utils::SetBreakable.find(src[i]) != string::npos) &&
	...
	// Clear memory.
	for(i = 0; i < facenum; i++){
		hb_buffer_destroy((fBuffs + i)->buffer);
	}
	free(fBuffs);
	if(bUseLayoutCache){
		hbPositionCache[wsrc] = result;
	}
...
template <class T> inline void _push_back(vector<T>& t1, T t2){
	t1.push_back(t2);
}
template <class T> inline void _setRect(T& t, float x, float y, float w, float h){}
inline void _setRect(ofRectangle& t, float x, float y, float w, float h){
	t.x = x;
	t.y = y;
	t.width = w, t.height = h;
}
template <class T> inline void _push_layout(T& t, float x, float y, int i){}
inline void _push_layout(vector<ofxFaceVec2>& t, float x, float y, int i){
	ofxFaceVec2 p;
	p.x = x;
	p.y = y;
	p.faceIndex = i;
...
template <class T> void ofxTrueTypeFontUL2::Impl::commonLayouts2(wstring src, float x, float y, float width,
																 float height, ul2_text_align textAlign,
																 vector<ofRectangle>* lineWidthList,
																 ul2_rendering_types type, T& result){
	...
	while(index < len){
		...
		if(setAlign){
			if(textAlign == UL2_TEXT_ALIGN_LEFT){
				if(bAlignByPixel || m_direction != HB_DIRECTION_LTR){
					ax = x - (*lineWidthList)[lineIndex].x;
				}
			}
				...
				if(bAlignByPixel || m_direction != HB_DIRECTION_RTL){
					ax = x + width - (*lineWidthList)[lineIndex].width - (*lineWidthList)[lineIndex].x;
				}
				else {
					ax = x + width;
				}
				...
				if(bAlignByPixel || m_direction != HB_DIRECTION_TTB){
					ay = y - (*lineWidthList)[lineIndex].y;
				}
			}
				...
				if(bAlignByPixel || m_direction != HB_DIRECTION_BTT){
					ay = y + height - (*lineWidthList)[lineIndex].height - (*lineWidthList)[lineIndex].y;
				}
				else {
					ay = y + height;
				}
			...
			if(boxWidth > 0){
				if(!bWordWrap || pos[index].breakable){
					...
					while(_index < len){
						...
						if((m_direction == HB_DIRECTION_LTR &&
							(__X + pos[_index].x1 + pos[_index].x_offset > boxWidth)) ||
						   (m_direction == HB_DIRECTION_RTL &&
							(__X + pos[_index].x2 + pos[_index].x_offset < -boxWidth)) ||
						   (m_direction == HB_DIRECTION_TTB &&
							(_Y + pos[_index].y1 + pos[_index].y_offset > boxWidth)) ||
						   (m_direction == HB_DIRECTION_BTT &&
							(_Y + pos[_index].y2 + pos[_index].y_offset < -boxWidth))){
							lineBreak = true;
							break;
						}
						else if(!bWordWrap || pos[_index].breakable){
							break;
						}
	...
	// -------------------------------------------------------------------------------
	if(type == UL2_GET_BOUNDINGBOX){
		_setRect(result, minx, miny, maxx - minx, maxy - miny);
	}
	if(type == UL2_GET_LINE_WIDTH){
		_push_back(result, ofRectangle(minx, miny, maxx - minx, maxy - miny));
	}
...
template <class T> void ofxTrueTypeFontUL2::Impl::commonLayouts(wstring src, float x, float y, float width,
																float height, int textAlign, ul2_rendering_types type,
																T& result){
	if(textAlign != UL2_TEXT_ALIGN_INVALID){
		...
		if(bAlignByPixel){
			...
			for(int i = 1; i < size; i++){
				...
				if(x2 < wList[i].x + wList[i].width){
					x2 = wList[i].x + wList[i].width;
				}
				if(y2 < wList[i].y + wList[i].height){
					y2 = wList[i].y + wList[i].height;
				}
			}
			vBox.x = x1;
			vBox.y = y1;
			...
			if(m_subDirection == HB_DIRECTION_TTB){
				vBox.y = -baseFontSize_;
			}
			else if(m_subDirection == HB_DIRECTION_BTT){
				vBox.y = -vBox.height;
			}
			else if(m_subDirection == HB_DIRECTION_LTR){
				vBox.x = -baseFontSize_ * .5f;
			}
			else if(m_subDirection == HB_DIRECTION_RTL){
				vBox.x = -(vBox.width - baseFontSize_ * .5f);
			}
		}
		if(bWritingHorizontal){
			if(textAlign & UL2_TEXT_ALIGN_LEFT){
				align = UL2_TEXT_ALIGN_LEFT;
			}
			else if(textAlign & UL2_TEXT_ALIGN_CENTER){
				align = UL2_TEXT_ALIGN_CENTER;
			}
			else if(textAlign & UL2_TEXT_ALIGN_RIGHT){
				align = UL2_TEXT_ALIGN_RIGHT;
			}
			if(textAlign & UL2_TEXT_ALIGN_V_TOP){
				y += -vBox.y;
			}
			else if(textAlign & UL2_TEXT_ALIGN_V_MIDDLE){
				y += (height - vBox.height) * .5f - vBox.y;
			}
			else if(textAlign & UL2_TEXT_ALIGN_V_BOTTOM){
				y += height - vBox.height - vBox.y;
			}
		}
		else {
			if(textAlign & UL2_TEXT_ALIGN_V_TOP){
				align = UL2_TEXT_ALIGN_V_TOP;
			}
			else if(textAlign & UL2_TEXT_ALIGN_V_MIDDLE){
				align = UL2_TEXT_ALIGN_V_MIDDLE;
			}
			else if(textAlign & UL2_TEXT_ALIGN_V_BOTTOM){
				align = UL2_TEXT_ALIGN_V_BOTTOM;
			}
			if(textAlign & UL2_TEXT_ALIGN_LEFT){
				x += -vBox.x;
			}
			else if(textAlign & UL2_TEXT_ALIGN_CENTER){
				x += (width - vBox.width) * .5f - vBox.x;
			}
			else if(textAlign & UL2_TEXT_ALIGN_RIGHT){
				x += width - vBox.width - vBox.x;
			}
		}
...
int ofxTrueTypeFontUL2::Impl::getCharID(const int codepoint, const unsigned int faceId){
	...
	for(; point != (int)loadedChars.size(); ++point){
		if(loadedChars[point].codepoint == codepoint && loadedChars[point].face == faceId){
			break;
		}
	}
...
void ofxTrueTypeFontUL2::Impl::loadChar(const int& charID){
	...
	if(err){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::loadFont - Error with FT_Load_Glyph %i: FT_Error = %d",
			loadedChars[i].codepoint,
			err);
	}
...
void ofxTrueTypeFontUL2::Impl::drawCharAsShape(int c, float x, float y){
	...
	if(strokeWidth > 0.0){
		charRef.setStrokeWidth(strokeWidth);
	}
	charRef.draw(x, y);
}
...
// -----------------------------------------------------------
bool ofxTrueTypeFontUL2::loadFont(string filename, float fontsize, bool _bAntiAliased, bool makeContours,
								  float _simplifyAmt, int _dpi, bool useTexture, string scriptTagName){
	return mImpl->implLoadFont(
		filename,
		fontsize,
		_bAntiAliased,
		makeContours,
...
bool ofxTrueTypeFontUL2::loadSubFont(string filename, float sizeRate, float baseLineRate, int unicodeRangeStart,
									 int unicodeRangeEnd, string scriptTagName){
...
void ofxTrueTypeFontUL2::reloadFont(){
	...
	mImpl->implLoadFont(
		mImpl->faces[0].filename,
		mImpl->faces[0].fontsize,
		mImpl->bAntiAliased_,
		mImpl->bMakeContours_,
		mImpl->simplifyAmt_,
		mImpl->dpi_,
		mImpl->bUseTexture_,
		mImpl->faces[0].scriptTagName);
	for(int i = 1; i < mImpl->faces.size(); i++){
		mImpl->implLoadSubFontVal(
			mImpl->faces[i].filename,
			mImpl->faces[i].fontsize,
			mImpl->faces[i].baseLine,
			mImpl->faces[i].unicodeRangeStart,
			mImpl->faces[i].unicodeRangeEnd,
			mImpl->faces[i].scriptTagName);
	}
}
...
void ofxTrueTypeFontUL2::addOTFeature(const char* feature_tag, unsigned int value, unsigned int start,
									  unsigned int end){
...
float ofxTrueTypeFontUL2::getAscenderHeight() const {
	if(mImpl->faces.size() > 0){
		return mImpl->faces[0].ascenderHeight;
	}
	else {
		return 0.0;
	}
...
float ofxTrueTypeFontUL2::getDescenderHeight() const {
	if(mImpl->faces.size() > 0){
		return mImpl->faces[0].descenderHeight;
	}
	else {
		return 0.0;
	}
...
void ofxTrueTypeFontUL2::useVrt2Layout(bool bUseVrt2Layout){
	...
	if(bUseVrt2Layout && !mImpl->bWritingHorizontal){
		addOTFeature("vrt2", 1);
	}
}
// -----------------------------------------------------------
...
// -----------------------------------------------------------
vector<ofRectangle> ofxTrueTypeFontUL2::getStringBoxes(wstring src, float x, float y, float width, float height,
													   int textAlign){
...
vector<ofRectangle> ofxTrueTypeFontUL2::getStringBoxes(string src, float x, float y, float width, float height,
													   int textAlign){
	return getStringBoxes(ul2_ttf_utils::convToWString(src), x, y, width, height, textAlign);
}

// -----------------------------------------------------------
ofRectangle ofxTrueTypeFontUL2::getStringBoundingBox(wstring src, float x, float y, float width, float height,
													 int textAlign){
...
ofRectangle ofxTrueTypeFontUL2::getStringBoundingBox(string src, float x, float y, float width, float height,
													 int textAlign){
	return getStringBoundingBox(ul2_ttf_utils::convToWString(src), x, y, width, height, textAlign);
}

// -----------------------------------------------------------
vector<ofPath> ofxTrueTypeFontUL2::getStringAsPoints(wstring src, float x, float y, float width, float height,
													 int textAlign){
	...
	if(!mImpl->bMakeContours_){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::drawStringAsShapes - Error : contours not created for this font - call loadFont with makeContours set to true");
...
vector<ofPath> ofxTrueTypeFontUL2::getStringAsPoints(string src, float x, float y, float width, float height,
													 int textAlign){
...
void ofxTrueTypeFontUL2::drawString(wstring src, float x, float y, float width, float height, int textAlign){
	if(!mImpl->bLoadedOk_){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::drawString - Error : font not allocated -- line %d in %s",
			__LINE__,
			__FILE__);
	...
	if(!mImpl->bUseTexture_){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::drawString - textures not created,  call loadFont with useTexture set to true");
...
void ofxTrueTypeFontUL2::drawStringAsShapes(wstring s, float x, float y, float width, float height, int textAlign){
	if(!mImpl->bLoadedOk_){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::drawStringAsShapes - Error : font not allocated -- line %d in %s",
			__LINE__,
			__FILE__);
...
// Ready for ofx3DFont
void ofxTrueTypeFontUL2::getLayoutData(vector<ofxFaceVec2>& facePosis, wstring s, float x, float y, float width,
									   float height, int textAlign){
	if(!mImpl->bLoadedOk_){
		ofLog(
			OF_LOG_ERROR,
			"ofxTrueTypeFontUL2::getLayoutData - Error : font not allocated -- line %d in %s",
			__LINE__,
			__FILE__);
...
void ofxTrueTypeFontUL2::getLayoutData(vector<ofxFaceVec2>& facePosis, string s, float x, float y, float width,
									   float height, int textAlign){
