...
namespace AngleApp
{
	...
	{
		std::lock_guard<std::mutex> guard(m_mutex);
		auto err = eglMakeCurrent(m_eglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
		if (!err) {
			auto str = eglErrorString(eglGetError());
			throw std::runtime_error("DeviceResouces: failed to set EGL context to NULL, " + str);
		}
	}
